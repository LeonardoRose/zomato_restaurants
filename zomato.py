...
def sidebar_filters(df):
    ...
    cities_list  = df.loc[:,'city'].sort_values().unique()
...
def sidebar_filters_test(df):
    ...
    country_multiselect_ = st.sidebar.multiselect(
                            'Choose the country:',
                            options = countries_list,
                                  )

    city_multiselect_ = st.sidebar.multiselect(
                            'Choose the city:',
                            options = cities_list,
                                  )
    
    if len(country_multiselect_) == 0:
        country_multiselect_ = df.loc[:,'country_name'].sort_values().unique()
    else:
        pass

    if len(city_multiselect_) == 0:
        city_multiselect_ = df.loc[:,'city'].sort_values().unique()
    else:
        pass

    if len(price_range_multiselect_) == 0:
        price_range_multiselect_ = df.loc[:,'price_range_type'].sort_values().unique()
    else:
        pass
    
    return country_multiselect_, city_multiselect_, price_range_multiselect_, price_slider_


def filter_df(price_range_multiselect, country_multiselect, city_multiselect, price_slider):
    selected_lines = (df['price_range_type'].isin(price_range_multiselect)) & (df['country_name'].isin(country_multiselect)) & (df['city'].isin(city_multiselect)) & (df['average_cost_for_two'] <= price_slider)
...
# SIDEBAR FILTERS
country_multiselect, city_multiselect, price_range_multiselect, price_slider = sidebar_filters(df)


sidebar_filters_test(df)
# FILTERED DF
df = filter_df(price_range_multiselect, country_multiselect, city_multiselect, price_slider)
